		...
		deviceSelectSource.callback = (action) => {
			if (action.options.method === 'spec') {
				for (const scr of action.options.screen) {
					...
					// Determine the correct preset target ('UP' or 'DOWN') based on the T-Bar's current position.
					// This is required by the new firmware API.
					...
					// Construct the new base path according to the AWJ Protocol Guide.
					const basePath = [
						'device',
					...
					if (screen.isAux && action.options['sourceBack'] !== 'keep')
						...
						for (const layer of action.options[`layer${screen.id}`]) {
							if (layer === 'NATIVE' && action.options['sourceNative'] !== 'keep') {
								...
								// Corrected path for setting a live layer's source.
								this.connection.sendWSmessage(
				...
				this.choices
					...
					.forEach((layer) => {
						// Determine the correct preset target ('UP' or 'DOWN') for the selected screen.
						const tbarState = this.state.get(
